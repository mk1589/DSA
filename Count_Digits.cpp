...
Dry-run of above algorithm: Consider an example, N = 58964. Initialize a variable digitsCount to zero which will store the count of digits.
Keep incrementing digitsCount until N is not zero, and reduce it by dividing by 10 at each step.
...
Analysis of above algorithm: You can clearly see that the number of operations performed in
the above solution is equal to the count of digits present in the number. So, the time complexity of the solution is O(digitsCount).
...
i.e., finding logarithmic and floor value. Therefore, the time complexity of the solution is O(1). */
